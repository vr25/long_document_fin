...
class ClassificationTrainer(Trainer):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if 'is_multilabel' in self.config and self.config['is_multilabel']:
                ...
                for tensor1, tensor2 in zip(torch.argmax(scores, dim=1), torch.argmax(batch.label.data, dim=1)):
                    if np.array_equal(tensor1, tensor2):
                        n_correct += 1
                loss = F.mse_loss(scores, batch.label.data)
    ...
    def train(self, epochs):
        ...
        header = '  Time Epoch Iteration Progress    (%Epoch)   Loss    mse'
        dev_header = '  Time Epoch Iteration Progress  Dev/Loss   Dev/mse'
        ...
        for epoch in range(1, epochs + 1):
            ...
            dev_acc, dev_precision, dev_recall, dev_f1, dev_loss, dev_mse = self.dev_evaluator.get_scores()[0]

            # Print validation results
            print('\n' + dev_header)
            print(self.dev_log_template.format(time.time() - self.start, epoch, self.iterations, epoch, epochs,
                                               dev_acc, dev_precision, dev_recall, dev_f1, dev_loss, dev_mse))
            ...
            if dev_loss > self.best_dev_loss:
                ...
                if self.iters_not_improved >= self.patience:
                    ...
                    print("Early Stopping. Epoch: {}, Best Dev F1: {}".format(epoch, self.best_dev_loss))
                    break
