...
def evaluate_split(model, processor, tokenizer, args, split='dev'):
    ...
    avg_loss, mse_loss = evaluator.get_scores(silent=True)[0]
    print('\n' + LOG_HEADER)
    print(LOG_TEMPLATE.format(split.upper(), avg_loss, mse_loss))
...
if __name__ == '__main__':
    ...
    pretrained_vocab_path = "bert-base-uncased"  #data/ftm/xgb_regr/longformer-esps/hedwig/hedwig-data/models/bert-base-uncased" #PRETRAINED_VOCAB_ARCHIVE_MAP[args.model]
    ...
    pretrained_model_path = "bert-base-uncased" #args.model if os.path.isfile(args.model) else PRETRAINED_MODEL_ARCHIVE_MAP[args.model]
    ...
    if not args.trained_model:
        if args.fp16:
            ...
            scheduler = get_linear_schedule_with_warmup(optimizer, num_training_steps=num_train_optimization_steps, num_warmup_steps=args.warmup_proportion * num_train_optimization_steps)
