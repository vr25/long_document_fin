#from modeling_bert import BertPreTrainedModel, BertConfig, BertModel
import numpy
...
from document_bert_architectures import DocumentBertMaxPool # DocumentBertLSTM, DocumentBertLinear, DocumentBertTransformer, DocumentBertMaxPool
...
def encode_documents(documents: list, tokenizer: BertTokenizer, max_input_length=512):
    ...
    print("max_sequences_per_document: ", max_sequences_per_document)
    ...
    #print("encode documents: ", output.shape)
    #print("document_seq_lengths: ", document_seq_lengths)
...
class BertForDocumentClassification():
    def __init__(self,args=None,
        ...
        if os.path.exists(self.args['bert_model_path']):
            ...
            config1 = BertConfig() #.from_pretrained(self.args['bert_model_path'])
            print("Debug config: ", config)
            m = BertModel(config1)
            config = m.config #.from_pretrained(self.args['bert_model_path'])
        ...
        self.bert_doc_classification = document_bert_architectures[self.args['architecture']](config) #(self.args['bert_model_path'], config=config)
    ...
    def fit(self, train, dev):
        ...
        #print("Debug fir train_documents: ", train_documents, train_labels)
        ...
        self.loss_function = torch.nn.MSELoss() #BCEWithLogitsLoss(pos_weight=loss_weight)
        ...
        for epoch in range(1,self.args['epochs']+1):
            ...
            for i in range(0, document_representations.shape[0], self.args['batch_size']):
                ...
                #if (i + self.args['batch_size']) % self.args['batch_size'] == 0:
                ...
                #print("debug - batch_document_tensors: ", batch_document_tensors.shape)
                #print("debug - batch_document_sequence_lengths: ", batch_document_sequence_lengths.shape)

                #print("debug forward: seq length: ", batch_document_sequence_lengths)
    ...
    def predict(self, data, threshold=0):
        ...
        #print("correct: ", correct_output, correct_output.shape)

        mse = mean_squared_error(correct_output.cpu().view(-1).numpy(), predictions.cpu().view(-1).numpy())
