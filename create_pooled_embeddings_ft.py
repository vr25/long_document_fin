...
tokenizer = AutoTokenizer.from_pretrained('bert-base-uncased') #allenai/longformer-base-4096') #bert-base-uncased')
...
train_seq = [[[101] + train_seq[j][i] + [102] if len(train_seq[j][i]) == max_length else train_seq[j][i] + [0] * (max_length+2-len(train_seq[j][i])) for i in range(len(train_seq[j]))] for j in range(len(train_seq))]
...
#Add [CLS], [SEP] and [PAD] tokens
val_seq = [[[101] + val_seq[j][i] + [102] if len(val_seq[j][i]) == max_length else val_seq[j][i] + [0] * (max_length+2-len(val_seq[j][i])) for i in range(len(val_seq[j]))] for j in range(len(val_seq))]
...
#Add [CLS], [SEP] and [PAD] tokens
test_seq = [[[101] + test_seq[j][i] + [102] if len(test_seq[j][i]) == max_length else test_seq[j][i] + [0] * (max_length+2-len(test_seq[j][i])) for i in range(len(test_seq[j]))] for j in range(len(test_seq))]
...
for epoch in range(epochs):
    ...
    if valid_loss < best_valid_loss:
        ...
        torch.save(model_to_save.state_dict(), 'saved_weights_bert_chunk_ft.pt')
...
test_error.to_csv("error_bert_chunk_ft.csv", index=False)

print("mse: ", mse)

mse_file = open("mse_bert_chunk_ft.txt", "w")
